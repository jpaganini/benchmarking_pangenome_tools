...
ncbi_sra_relation <- read.csv("../../2023_10_raw_data/results/stec_info/biosample_strains_o157_h7.csv", sep=",", header=FALSE)
...
# 2. Build the order of stx_profile based on stx_count (1 → 4)
...
#write.csv(all_metadata, '../results/Supplementary_table_S2_all_metadata.csv',  row.names = FALSE)
```
...
###9. Plot More repeats data
```{r fig.width=5, fig.height=3, dpi=300}

repeat_data_long <- repeat_data %>%
  pivot_longer(
    cols = -c(isolate, Serotype, Serotype_plot),
    names_to = "metric",
    values_to = "value"
  )


repeat_distribution <- ggplot(repeat_data_long, 
                        aes(x = Serotype_plot, y = as.numeric(value))) +
  geom_violin(aes(fill = Serotype_plot),alpha = 0.8, trim = TRUE,scale = "width") +
  geom_boxplot(position = position_dodge(width = 0.8), width = 0.2,outlier.shape = NA) +
  coord_cartesian(ylim = c(0, 0.3)) +
  #geom_jitter(aes(color = dot_colour), alpha = 0.3, position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.8)) +
  theme_bw() +
  # Uncomment and customize color if needed
  scale_fill_manual(values = c('O157:H7' = '#33A02C','Other' = '#D9D9D9')) +
  theme(
  ...
  xlab('Serotype') + 
  ylab('Value')+
  facet_wrap(~ metric, scales="fixed") + theme(strip.text.x = element_text(size = 14))

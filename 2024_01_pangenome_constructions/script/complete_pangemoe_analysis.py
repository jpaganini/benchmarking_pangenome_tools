...
cuttlefish_file_path = os.path.join(input_path, 'cuttlefish.gfa1')
...
def pangraph_length(gfa_file):
    ...
    with open(gfa_file, 'r') as file:
        ...
        for line in tqdm(file, desc="Processing Pangraph sequence length", unit=" lines", total=total_lines):
            if line.startswith('S'):  # Node 
                _, _, _, _, length = line.strip().split('\t')
...
panaroo_node_counts_df = pd.DataFrame(panaroo_node_counts, columns=['No. of nodes (log10)'])
panaroo_node_counts_df['Tool'] = 'panaroo'
PPanGGOLiN_node_counts_df = pd.DataFrame(PPanGGOLiN_node_counts, columns=['No. of nodes (log10)'])
PPanGGOLiN_node_counts_df['Tool'] = 'PPanGGOLiN'
ggcaller_node_counts_df = pd.DataFrame(ggcaller_node_counts, columns=['No. of nodes (log10)'])
ggcaller_node_counts_df['Tool'] = 'ggcaller'
bifrost_node_counts_df = pd.DataFrame(bifrost_node_counts, columns=['No. of nodes (log10)'])
bifrost_node_counts_df['Tool'] = 'bifrost'
cuttlefish_node_counts_df = pd.DataFrame(cuttlefish_node_counts, columns=['No. of nodes (log10)'])
cuttlefish_node_counts_df['Tool'] = 'cuttlefish'
pangraph_node_counts_df = pd.DataFrame(pangraph_node_counts, columns=['No. of nodes (log10)'])
...
for i, group in enumerate(subgraph_df['Tool'].unique()):
    ...
    # Subset only observation from the group
    y = subgraph_df[subgraph_df['Tool'] == group]['No. of nodes (log10)'] 
